...
Updated September 9, 2026. Phase 1 is implemented and the completed starter has been extracted into `merxet-storefront-template`: a configurable storefront, maintained catalog and commerce helpers, static builds, and local acceptance checks. `merxet-promo` continues as the existing catalog application. Automated AI customization, the builder, sandbox execution, and publication described below remain planned. See the [template README](./merxet-storefront-template/README.md) for local usage.
...
Phase 1 is complete, and the standalone template extraction is the first preparation step for Phase 2. Next prove the agent can produce two different designs from copies of `merxet-storefront-template` in Railway Sandboxes. After the feasibility trial, create `merxet-storefront-builder` to automate the queue, sandbox lifecycle, revision management, and publication. The maintained project layout now includes the extracted template and the planned builder alongside the existing promo application.
