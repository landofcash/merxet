...
function decodeHashBytes(payloadHash: string): Uint8Array {
  const hashBytes = payloadHash.startsWith("0x") ? hexToBytes(payloadHash) : b64ToBytes(payloadHash);
...
function encodeReferencePayload(reference: HcsReferencePayload): Uint8Array {
  ...
  const payloadHashBytes = decodeHashBytes(reference.payloadHash);
