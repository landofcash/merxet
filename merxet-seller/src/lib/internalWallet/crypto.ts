...
export async function decryptSecretMaterial(
  ...
  const salt = b64ToBytes(encryptedSecret.saltBase64);
  const iv = b64ToBytes(encryptedSecret.ivBase64);
  const cipherText = b64ToBytes(encryptedSecret.cipherTextBase64);
