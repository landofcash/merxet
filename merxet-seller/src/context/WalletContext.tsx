import {createContext, useContext, useState, useCallback, type ReactNode, useEffect, useMemo, useRef} from 'react'
...
import {hashpackWalletAdapter} from './wallet/adapters/hashpackWalletAdapter'
...
export function WalletProvider({children}: { children: ReactNode }) {
  ...
  const walletAdapterForUi: WalletAdapter | null = useMemo(() => {
  ...
  const signMessage = useCallback(async (dataToSign: string, message: string): Promise<Uint8Array> => {
    ...
    const adapter = walletAdapterForUi
    if (!adapter) throw new Error('No active wallet adapter')
    return await adapter.signMessage(dataToSign, message)
  }, [walletKind, walletAdapterForUi])
