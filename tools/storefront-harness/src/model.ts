...
export function modelSettings() {
  ...
  if(!key || !model) throw new Error('Set OPENAI_API_KEY and an explicit OPENAI_MODEL in the local harness environment.');
...
export async function requestEdits(source:FileSet,input:GenerationInput,feedback='',signal?:AbortSignal,transport:typeof fetch=fetch) {
  const settings=modelSettings();
  const prompt=generationPrompt(source,input,feedback);
